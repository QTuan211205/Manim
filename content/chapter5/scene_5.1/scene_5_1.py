...
class Scene5_1(Scene):
    ...
    def make_state_machine(self):
        title = create_text("Constrained state machine", font_size=5.2, color=BLUE_A)
        states = VGroup()
    ...
    def make_conveyor(self):
        title = create_text("Speculative pipeline", font_size=5.4, color=ORANGE)
    ...
    def construct(self):
        ...
        recap_title = create_markup_text("<b>Journey Recap</b>", font_size=13, color=YELLOW).move_to(UP * 2.25)
        journey = create_markup_text(
            "Primitive token generators  ->  Search/self-correcting Meta-Generation  ->  Hardware systems optimization",
            font_size=7.3,
            color=GRAY_A,
        ).move_to(DOWN * 2.2)
        ...
        nouha = self.quote_box(
            ...
            "<b>1. Snowballing of error</b>: a small early error derails all later reasoning.\n"
            "<b>2. Difficulty of look-ahead</b>: hard to look ahead across many consequences.",
            "#6BCB77",
            height=1.65,
            y=0.85,
        )
        structure = self.quote_box(
            "Role of Meta-Generation",
            "Tree search, backtracking, and PRM process scoring are an additional structural layer\nthat expands model capability.",
            GREEN,
            height=1.15,
            y=-1.0,
        )
        ...
        noam = self.quote_box(
            ...
            "A question often costs only a fraction of a cent; but for important problems\n"
            "like the Riemann hypothesis or drug discovery, people may pay thousands\n"
            "to a million dollars for an accurate answer.",
            "#FF6B6B",
            height=1.95,
            y=0.75,
        )
        scale = self.quote_box(
            ...
            "An 8-order-of-magnitude gap leaves a large space for further inference scaling.",
            "#FF6B6B",
            height=0.95,
            y=-1.35,
        )
        ...
        final_text = create_markup_text(
            ...
            "The future is not only about building larger models,\n"
            "but about designing intelligent systems\n"
            "that know how to think and allocate inference resources optimally.",
            font_size=12,
            color=WHITE,
            line_spacing=1.35
        )
        ...
        final_outro_txt = create_markup_text(
            ...
            "All slides and source code are linked in the description.",
