...
SCENE_1_3_VOICEOVERS = tuple(SCENE_1_3_DURATIONS)


def validate_scene_voiceover_files():
    available = sorted(path.name for path in VOICEOVER_DIR.glob("sc13_*.mp3"))
    expected = sorted(SCENE_1_3_VOICEOVERS)
    if available != expected:
        missing = sorted(set(expected) - set(available))
        extra = sorted(set(available) - set(expected))
        raise FileNotFoundError(
            f"Scene 1.3 voiceover mismatch. Missing: {missing or 'none'}; extra: {extra or 'none'}"
        )


def add_voiceover(scene, filename, time_offset=0.0, duration=0.0):
    if filename not in SCENE_1_3_DURATIONS:
        raise KeyError(f"Unexpected Scene 1.3 voiceover: {filename}")
    if not (VOICEOVER_DIR / filename).exists():
        raise FileNotFoundError(f"Missing Scene 1.3 voiceover file: {filename}")
...
def schedule_scene_voiceovers(scene):
    ...
    voiceover_end = 0.0
    for filename, duration in SCENE_1_3_DURATIONS.items():
...
def assert_all_scene_voiceovers_played(scene):
    ...
    if played != SCENE_1_3_VOICEOVERS:
        missing = [filename for filename in SCENE_1_3_VOICEOVERS if filename not in played]
        raise RuntimeError(
            f"Scene 1.3 did not schedule every voiceover. Played: {played}; missing: {missing or 'none'}"
        )
...
class Scene1_3(Scene):
    def construct(self):
        ...
        # Note: visual/narration alignment comment translated from Vietnamese.
        main_title = create_text("Methods for Scaling Inference-Time Compute", font_size=20, color=BLUE_A)
        ...
        for idx, (box, label) in enumerate(zip(code_boxes, code_labels)):
            if idx == 2: # Note: visual/narration alignment comment translated from Vietnamese.
        ...
        # Note: visual/narration alignment comment translated from Vietnamese.
        # =====================================================================
        summary_title = create_text("Inference-Time Scaling = Extra Tokens, Parallel Generation & Tool Use", font_size=11, color=BLUE_A)
        ...
        card1_title = create_text("1. Generate Reasoning", font_size=13, color=GREEN_A).next_to(card1.get_top(), DOWN, buff=0.15)
        card1_desc = create_text("- Chain of Thought (CoT)\n- Allocate extra tokens\n- Increase expressiveness\n- Optimize reasoning", font_size=10, color=WHITE).next_to(card1_title, DOWN, buff=0.25, aligned_edge=LEFT)
        ...
        card2_title = create_text("2. Search & Voting", font_size=13, color=YELLOW).next_to(card2.get_top(), DOWN, buff=0.15)
        card2_desc = create_text("- Generate N samples in parallel\n- Score with a reward model\n- Majority voting\n- Minimum Bayes risk", font_size=10, color=WHITE).next_to(card2_title, DOWN, buff=0.25, aligned_edge=LEFT)
        ...
        card3_title = create_text("3. Compound AI System", font_size=13, color=BLUE_A).next_to(card3.get_top(), DOWN, buff=0.15)
        card3_desc = create_text("- Call external tools\n- Optimize resources\n- Use calculators / write code\n- Reduce arithmetic errors", font_size=10, color=WHITE).next_to(card3_title, DOWN, buff=0.25, aligned_edge=LEFT)
